...
main (int argc, char *argv[])
{
  ...
  NodeContainer borda = NodeContainer (nodes.Get(0), nodes.Get(4), nodes.Get(8), nodes.Get(12), server.Get(0));
...
// I-D-F
  ...
  Devdf = pointToPoint.Install (df); // - OK 
  Devfi = pointToPoint.Install (fi); // - OK 
  DeviServer = pointToPoint.Install (iServer); // - ERRO
...
// BORDA DEV
  ...
  BordaDev = wifi.Install (phy, mac, borda);
...
if (Random) {
  ...
  server.Get (0)->GetObject<MobilityModel> ()->SetPosition (Vector (-20, 100, 10));
  server.Get (0)->GetObject<ConstantVelocityMobilityModel> ()->SetVelocity (Vector (0, speed, 0));
  ...
  Ipv4InterfaceContainer Interborda = address.Assign(BordaDev);
  ...
  UdpEchoClientHelper echoClient (Interborda.GetAddress (4), 9);
